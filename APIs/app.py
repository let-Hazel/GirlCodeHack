...
def create_user_route():
    ...
    if not name or not surname or not email or not password:
        return jsonify({"error": "Name, surname, email and password are required"}), 400

    try:
        user_id = create_user(name, surname, email, password, phone, location)
...
def login():
    data = request.get_json()
    user = log_in(data.get("email"), data.get("password"))
    if not user:
...
def create_skill_route():
    ...
    return jsonify({"message": "Skill created", "skill_id": skill_id}), 201
...
def requests_for_provider_route(provider_id):
    return jsonify(get_requests_for_provider(provider_id)), 200


@app.get("/requests/requester/<requester_id>")
def requests_for_requester_route(requester_id):
    return jsonify(get_requests_for_requester(requester_id)), 200
...
def ratings_for_provider_route(provider_id):
    return jsonify(get_ratings_for_provider(provider_id)), 200
